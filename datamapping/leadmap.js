...
const findOrCreateProductStage = async (stageData) => {
  try {
    const productId = stageData.product_id;
    const productStage = await ProductStage.findOne({ name: stageData.name, product_id: productId });
    ...
    if (!productStage) {
      ...
      return newProductStage._id;
    }

    return productStage._id;
  } catch (error) {
    console.error('Error finding or creating product stage:', error);
    throw error;
  }
};
...
const insertLeads = async () => {
  try {
    ...
    for (const productData of productsData) {
      ...
      productMap.set(productData.id, productId);
    }
    ...
    for (const lead of leadsData) {
      ...
      const productId = productMap.get(lead.products) || null;
      const productStageId = await findOrCreateProductStage({
        name: stagesMap.get(lead.stage_id).name,
        product_id: productId,
        order: stagesMap.get(lead.stage_id).order,
        created_at: stagesMap.get(lead.stage_id).created_at,
        updated_at: stagesMap.get(lead.stage_id).updated_at,
      });
